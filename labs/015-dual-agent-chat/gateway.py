...
Reads MODEL_PROXY_* from env or <repo>/.env.model-proxy.
...
WEB_DIST = LAB_DIR / "web" / "dist"
ENV_FILE = REPO_ROOT / ".env.model-proxy"
...
def proxy_target() -> tuple[str, str]:
    ...
    if not url or not key:
        raise RuntimeError(
            ...
            f"  cd {REPO_ROOT} && kaggle b auth -y --env-file .env.model-proxy"
        )
